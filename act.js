(() => {
  ...
  function createDropdown() {
    ...
    box.innerHTML = `
      <div class="dropdown-grid">
        <label>Категория<select id="workCategory"></select></label>
        <label>Работа<select id="workSelect"></select></label>
        <label>Кол-во<input id="workQty" type="number" min="1" value="1" inputmode="numeric"></label>
      </div>
      <button class="main-btn dropdown-add" id="addWork" type="button">Добавить</button>
  ...
  function addWork() {
    const serviceId = $('workSelect')?.value;
    const qtyToAdd = Math.max(1, money($('workQty')?.value || 1));
    const input = document.querySelector(`[data-qty='${serviceId}']`);
    const currentQty = money(input?.value || 0);
    setServiceQty(serviceId, currentQty + qtyToAdd);
  ...
  function updatePicked() {
    ...
    box.innerHTML = items.map((item) => `
      <div class="picked-row">
        <div><div class="picked-title">${item.title}</div><div class="picked-meta">${item.qty} ${item.unit} × ${rub(item.price)}</div></div>
        <div class="picked-sum">${rub(item.subtotal)}</div>
        <button class="picked-remove" type="button" data-remove-work="${item.id}">×</button>
      </div>
    `).join('');
  }
  ...
  function createSettingsToggle() {
    ...
    button.type = 'button';
    button.className = 'ghost settings-toggle';
    button.textContent = 'Доп. настройки';
  ...
  function createActForm() {
    ...
    panel.innerHTML = `
      ...
      <div class="act-content">
        <div class="act-grid">
          <label>Номер акта<input id="actNumber" value="${actDefaults.number}"></label>
          <label>Дата<input id="actDate" type="date" value="${actDefaults.date}"></label>
          ...
          <label>Исполнитель<input id="actExecutor" value="${actDefaults.executor}"></label>
          <label>Телефон<input id="actPhone" value="${actDefaults.phone}"></label>
          <label>Гарантия, дней<input id="actWarranty" type="number" min="0" value="${actDefaults.warranty}"></label>
          <label class="wide">Email клиента<input id="actEmail" type="email" placeholder="client@mail.ru"></label>
          <label class="wide">Рекомендованные работы<textarea id="actRecommended">${actDefaults.recommended}</textarea></label>
        </div>
  ...
  function buildAct(show = true) {
    ...
    if (show) {
      $('actPreview')?.classList.add('open');
      $('actPreview')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }

  function sendMail() {
    buildAct(false);
    const subject = encodeURIComponent(`Акт выполненных работ № ${value('actNumber')}`);
    const body = encodeURIComponent(actPlainText());
    window.location.href = `mailto:${encodeURIComponent(value('actEmail').trim())}?subject=${subject}&body=${body}`;
  }
  ...
  function bind() {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (remove) setServiceQty(remove.dataset.removeWork, 0);
      if (event.target.matches('[data-plus], [data-minus], #resetBtn')) setTimeout(updatePicked, 0);
    });
    document.addEventListener('input', (event) => {
      if (event.target.matches('[data-qty], #visitPrice, #minimumPrice, #materialsExtra, #complexity, #urgency, #discount')) {
        setTimeout(() => { updatePicked(); buildAct(false); }, 0);
      }
    });
